#include "Teacher.h"
	...
	Teacher::Teacher
	(
		const string& name, unsigned int age, const string& specialty, unsigned int xp , int evil
	) :Human(name, age)
	{
	...
	ostream & operator<<(ostream & os, Teacher & obj)
	{
		...
		os << (Human)obj;
		return os << "Специальность: " << obj.get_specialty() << " , опыт преподавания: " << obj.get_xp() << " лет, " << "уровень злости: " << obj.get_evil() << "%." << endl;
	}
